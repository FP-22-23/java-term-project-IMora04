...
public class Accident implements Comparable<Accident> {
	...
	private List<String> info; // LIST PROPERTY: Extra information about the accident (type, subtype, subzone,
								// moment, wind)

	// 1ST CONSTRUCTOR: Receives a value for every basic property

	public Accident(LocalDate date, String location, Victims victims, Integer speed, Boolean escapist, Climate climate,
			List<String> info) {

		// RESTRICTION 1: The accident's date must be lower or equal than today's date

		Checkers.check("Invalid date", date.compareTo(LocalDate.now()) > 0);

		// RESTRICTION 2: The maximum speed cannot be negative.

		Checkers.check("Invalid speed", speed != null && speed <= 0);
	...
	public Accident(String s) {
		...
		Checkers.check("Incorrect string format", values.length == 15);
		...
		victims = new Victims(Integer.valueOf(values[4].trim()), Integer.valueOf(values[5].trim()),
				Integer.valueOf(values[6].trim()));
		...
		case "good weather":
			climate = Climate.GOOD_WEATHER;
			break;
		case "heavy rain":
			climate = Climate.HEAVY_RAIN;
			break;
		case "light rain":
			climate = Climate.LIGHT_RAIN;
			break;
		case "hail":
			climate = Climate.HAIL;
			break;
		case "snowing":
			climate = Climate.SNOWING;
			break;
		default:
			climate = Climate.UNSPECIFIED;
		}
		info = List.of(values[14].trim(), // type
	...
	// DERIVED PROPERTY: Proportion of deaths, with respect to the total number of
	// victims (between 0 and 1)
	...
	// STRING REPRESENTATION: Represent every property. Every property of the type
	// Victims is also represented

	public String toString() {
		return "Accident [DeathsProportion=" + getDeathsProportion() + ", Victims=" + getVictims() + ", Date="
				+ getDate() + ", Location=" + getLocation() + ", Escapist=" + getEscapist() + ", Speed=" + getSpeed()
				+ ", Climate=" + getClimate() + ", Info=" + getInfo() + "]";
	}

	// EQUALITY CRITERION: An accident is equal to another if every basic property
	// is equal
	...
	// ORDER CRITERION: Ordered by date. If equal, ordered by location
	// (alphabetically)
